...
#### burn
When a 'burn' takes place, funds are sent to a specific address that no one can access. Because no one can ever access funds sent to a burn address, it reduces the [circulating supply](#circulating-supply).
